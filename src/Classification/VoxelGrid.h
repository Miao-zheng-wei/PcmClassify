...
public:
	...
	void VoxelGrid_ApplyFilter(std::vector<PointT> &OutPointCloud,
		...
		std::vector<std::pair<unsigned int, unsigned int> > &first_and_last_indices_vector,GenericProgressCallback *_pDlg = 0);
	...
	GenericProgressCallback *_pDlg)
{
	...
	while (index < index_vector.size())
	{
		...
		if (i - index >= min_points_per_voxel_)
		{
			...
			OutPointCloud.emplace_back(InputCloudPoint[index_vector[index].cloud_point_index]);
		}
		index = i;
	}
	...
	//	//	z_Sum += InputCloudPoint[index_vector[li].cloud_point_index].z;
	//	//}
	//	PointCloud.x = InputCloudPoint[index_vector[first_index].cloud_point_index];
	//	PointCloud.y = InputCloudPoint[index_vector[first_index].cloud_point_index];
	//	PointCloud.z = InputCloudPoint[index_vector[first_index].cloud_point_index];
	//	OutPointCloud.emplace_back(PointCloud);
	//}
